    ...
    public List<EstudianteDTO> getEstudiantesSorted(String campo) {
        try {
            if (!camposOrdenados.contains(campo.toLowerCase())) {
                throw new IllegalArgumentException("El campo pasado como parametro no existe");
            }
            EntityManager em = JPAUtil.getEntityManager();
            String jpql = "SELECT new micro.example.dto.EstudianteDTO(" +
                    ...
                    "FROM Estudiante e ORDER BY LOWER(e." + campo + ")";
            List<EstudianteDTO> estudiantes = em.createQuery(jpql, EstudianteDTO.class)
                    .getResultList();
            em.close();
            return estudiantes;
        } catch (Exception e) {
            throw new RuntimeException("*** Error al obtener estudiantes ordenador por " + campo + " ***", e);
        }
    }
